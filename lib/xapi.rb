...
module Xapi
  ...
  def self.create_activity(opts={})
    activity_definition = ActivityDefinition.new(name: {lang(opts[:name]) => opts[:name]}, type: opts[:type])
    activity_definition.description = {lang(opts[:description]) => opts[:description]} if opts[:description].present?
  ...
  def self.create_result(opts={})
    ...
    result = Result.new(duration: opts[:duration].presence, score: score)
